...
export default function AddNodeScreen() {
  ...
  const initialType =
    typeof params.type === "string" ? params.type : "Products";
  ...
  const [generatedPayload, setGeneratedPayload] =
    useState<ProductPayload | null>(null);
  ...
  const populateFormFromPayload = (p: ProductPayload) => {
    ...
    if (p.categorization) {
      ...
      if (p.categorization.subcategory)
        setSubcategory(p.categorization.subcategory);
  ...
  const handleAiRefine = async () => {
    ...
    setIsRefining(true);
    try {
      const refined = await refineProductPayload(
        generatedPayload,
  ...
  const saveInlineEdit = () => {
    ...
    switch (editingField) {
      case "title":
        ...
        setEditedTitle(editingValue);
        break;
      case "universal_code":
        updated.universal_code = editingValue;
        setEditedCode(editingValue);
        break;
      case "description":
        ...
        break;
      case "brand":
        updated.brand = editingValue;
        setBrand(editingValue);
        break;
      case "availability":
        updated.availability = editingValue;
        setAvailability(editingValue);
        break;
      case "delivery":
        updated.delivery = editingValue;
        setDelivery(editingValue);
        break;
      case "return_policy":
        updated.return_policy = editingValue;
        setReturnPolicy(editingValue);
        break;
      case "price":
        const amt = parseFloat(editingValue);
        if (!isNaN(amt)) {
          updated.price = { ...updated.price, amount: amt };
          setPriceAmount(editingValue);
        }
        break;
      case "category":
        updated.categorization = {
          ...updated.categorization,
          category: editingValue,
        };
  ...
  const buildPayload = (): string => {
    ...
    if (category.trim()) {
      ...
      if (subcategory.trim())
        payload.categorization.subcategory = subcategory.trim();
      if (tags.trim())
        payload.categorization.tags = tags
          .split(",")
          .map((t) => t.trim())
          .filter(Boolean);
    }
  ...
  const handleSave = async () => {
    ...
    if (!finalTitle || !finalCode) {
      Alert.alert(
        "Missing Fields",
        "Please ensure Title and Universal Code are set.",
      );
      return;
    }
    setIsSaving(true);
        ...
        {generatedPayload && nodeType === "Products" && (
          <Animated.View
            style={[
              s.propertiesSection,
              {
                ...
                transform: [
                  {
                    translateY: previewFadeAnim.interpolate({
                      inputRange: [0, 1],
                      outputRange: [12, 0],
                    }),
                  },
          ...
          >
            ...
            <View style={s.propertiesTable}>
              {generatedPayload.brand &&
                renderProperty("🏷️", "Brand", generatedPayload.brand, "brand")}
              {generatedPayload.price?.amount != null &&
                renderProperty(
                  "💰",
                  "Price",
              ...
              {generatedPayload.categorization?.category &&
                renderProperty(
                  "📂",
                  "Category",
              ...
              {generatedPayload.return_policy &&
                renderProperty(
                  "↩️",
                  "Returns",
                  generatedPayload.return_policy,
            ...
            {/* Tags */}
            {generatedPayload.categorization?.tags &&
              generatedPayload.categorization.tags.length > 0 && (
            ...
            {/* Options */}
            {generatedPayload.options &&
              generatedPayload.options.length > 0 && (
            ...
            {/* Specifications */}
            {generatedPayload.specifications &&
              Object.keys(generatedPayload.specifications).length > 0 && (
                <View style={s.tableBlock}>
                  <Text style={s.tableBlockTitle}>Specifications</Text>
                  {Object.entries(generatedPayload.specifications).map(
                    ([k, v], i) => (
                      <View key={i} style={s.tableRow}>
