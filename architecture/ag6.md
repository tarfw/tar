...
| ------------------- | ------ | ----------------- | --------- | ------------------------------------- | -------------------------------------------------------- |
| **Inventory Stock** | `mass` | `tenant db`       | `stock`   | `"kitchen"` or `"warehouse"`          | `qty = 80`, `geo = "Adyar"`                              |
| **Selling Price**   | `mass` | `tenant db`       | `price`   | `"dine_in"`, `"online"`, `"delivery"` | `value = 180.00`                                         |
| **Shift Schedule**  | `mass` | `tenant db`       | `slot`    | `"shift_am"` or `"shift_pm"`          | `start = "08:00"`, `end = "16:00"`                       |
...
| **Discount Coupon** | `mass` | `tenant db`       | `coupon`  | `"promo_may"`                         | `qty = 100` (uses remaining)                             |
| **Loyalty Balance** | `mass` | `tenant db`       | `loyalty` | `"customer_points"`                   | `qty = 450`                                              |
...
| ---------------------- | -------- | ----------------- | ----------------- | ------------- | ------------------------- | -------------------------------------------- |
| **Dine-in Sale**       | `motion` | `tenant db`       | `1` (SALE)        | `"DONE"`      | `value_delta = 540`       | `{ "items": ["biryani x3"] }`                |
| **Inventory Restock**  | `motion` | `tenant db`       | `100` (UPDATE)    | `"DONE"`      | `qty_delta = +50`         | `{ "supplier": "A1 Poultry" }`               |
| **Clock-In Shift**     | `motion` | `tenant db`       | `100` (UPDATE)    | `"DONE"`      | —                         | `{ "clock_in_time": "08:02" }`               |
| **KDS Kitchen Order**  | `motion` | `tenant db`       | `100` (UPDATE)    | `"PENDING"`   | —                         | `{ "table": 4, "kds_screen": "main" }`       |
| **KDS Order Ready**    | `motion` | `tenant db`       | `100` (UPDATE)    | `"DONE"`      | —                         | `{ "chef": "kumar", "prep_time_secs": 640 }` |
| **Task Started**       | `motion` | `tenant db`       | `200` (TASK)      | `"STARTED"`   | —                         | `{ "assigned_to": "priya" }`                 |
| **Task Completed**     | `motion` | `tenant db`       | `200` (TASK)      | `"COMPLETED"` | —                         | `{ "completed_at": "11:30" }`                |
...
| ------------------------ | ---------- | ----------------- | ------------------- | --------------------- | ---------------- |
| **Store sells product**  | `relation` | `tenant db`       | `mat_store_adyar`   | `mat_product_biryani` | `"SELLS"`        |
| **Staff works at store** | `relation` | `tenant db`       | `mat_person_kumar`  | `mat_store_adyar`     | `"WORKS_AT"`     |
| **Chef cooks product**   | `relation` | `tenant db`       | `mat_person_kumar`  | `mat_product_biryani` | `"COOKS"`        |
| **Delivery driver role** | `relation` | `tenant db`       | `mat_person_ravi`   | `mat_store_adyar`     | `"DELIVERS_FOR"` |
...
| **Sales Logs (Checkout)**       | `motion` (action = `1` / `SALE`)                       | `tenant db`                     | **Manual GUI**     | Tapping items on a POS screen or scanning barcodes is instantaneous, deterministic, and doesn't incur LLM latency/cost.                        | **Zero**             |
| **Voice Order Taking**          | `motion` (action = `1` / `SALE`)                       | `tenant db`                     | **AI / LLM**       | Custom speech-to-text parsing extracts item quantities and modifiers (_"2 spicy chicken biryani and a lassi"_) automatically.                  | **Moderate**         |
| **Stock Adjustments**           | `mass` (type = `stock`)                                | `tenant db`                     | **Manual GUI**     | A simple `+` / `-` qty stepper or numeric input field on the screen is instant and free of API costs.                                          | **Zero**             |
| **Shift Scheduling**            | `mass` (type = `slot`, scope = `shift`)                | `tenant db`                     | **Manual GUI**     | Best managed via a standard calendar UI, shift calendar templates, or simple toggle cards.                                                     | **Zero**             |
| **Completing Tasks**            | `motion` (action = `200` / `TASK`)                     | `user private db` / `tenant db` | **Manual GUI**     | Clicking a checkbox to log a task state mutation to `COMPLETED` is much faster and cheaper than telling an agent to do it.                     | **Zero**             |
| **Complex Multi-entity Events** | Multiple (`matter` + `mass` + `relation`)              | `tenant db` / `user private db` | **AI / LLM**       | E.g., _"Set up a recurring shift for Kumar at Adyar from 8am to 4pm every Monday."_ LLMs handle multi-row relational mapping easily in one go. | **High Value**       |
...
- **Database Topology Segregation**:
  - `home.tsx` pulls from both `getUserDb()` (**User Private DB**) and `getTenantDb()` (**Tenant DB**), implementing the database separation rules documented in `ag6.md`:
    - Personal notes, reminders, and user tasks come from `user private db`.
    - Shared business sales, shifts, KDS entries, and inventory levels come from `tenant db`.
