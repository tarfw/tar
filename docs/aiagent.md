...
| :--- | :--- | :--- | :--- |
| **UI** | **Action** | Visual forms representing tools. | *"Browse Actions"* |
...
### Industry Terminology Alignment (MCP & Google Cloud)

| Concept | `tar` Framework | MCP (Anthropic / Industry standard) | Google Cloud (Gemini Enterprise Agent Platform) |
| :--- | :--- | :--- | :--- |
| **Definition** | `ActionDef` / Action | **Tool** (Function schema + parameters) | **Skill** (Packaged `SKILL.md` + scripts + assets) |
| **Execution** | Action Handler / Execute | Tool Call (`tools/call`) | Skill Execution |
| **Workflow** | **Flow** | *None (Application-specific)* | **Workflow** / Chained Agents |
...
* **Tool:** A single functional block (e.g., SQLite query, HTTP request). *Maps directly to an **MCP Tool** (`tools/call`)*.
* **Workflow (Flow):** A deterministic, chained sequence of tools (e.g., Task A $\rightarrow$ Task B $\rightarrow$ Task C). *Executed as a sequential pipeline of **MCP Tool Calls**.*
* **Agent:** An LLM-orchestrated executor that uses planning, memory, and tool-calling to solve open-ended tasks dynamically. *Acts as an **MCP Client** session that dynamically queries **MCP Resources** and invokes **MCP Tools**.*
