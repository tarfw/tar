...
@theme {
  ...
  /* Spacing
     Tailwind v4's default spacing scale is a 4px base (--spacing: 0.25rem)
     and already matches the design's tokens (4/8/12/16/20/24/32/40/48/64/72/96).
     Do NOT redeclare --spacing-* here: it overrides the utility scale, so
     utilities no longer follow standard semantics (e.g. h-20 becomes 20px
     instead of 80px, px-8 becomes 8px instead of 32px). Use the default scale
     and write utilities by their standard names (p-4 = 16px, h-20 = 80px, etc.).
     --spacing-36 and --spacing-120 are not part of the default scale; use
     arbitrary values (p-[36px], p-[120px]) if those exact sizes are needed. */
