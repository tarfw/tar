...
/**
 * Convert a sales lead to a customer or opportunity.
 * @param input - Conversion details
 * @param input.leadId - Lead matter ID
 * @param input.dealValue - Deal value
 * @param input.notes - Optional notes
 * @param input.scope - Tenant scope
 * @returns Whether lead was converted and its ID
 */
export async function actionConvertLead(input: {
  leadId: string;
  dealValue: number;
  notes?: string;
  scope: string;
}): Promise<{ converted: boolean; leadId: string }> {
