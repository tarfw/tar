...
/**
 * Workspace — Per-workspace stock, services, config (matter only).
 * Scope prefix: w:
 */
...
/**
 * Order — Per-order state machine + payment for complex orders.
...
export class Order extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/create' && request.method === 'POST') {
      ...
      // Reserve stock in Workspace
      const workspaceId = this.env.WORKSPACE.idFromName(body.storeSlug);
      const workspaceDO = this.env.WORKSPACE.get(workspaceId);
    ...
    if (path === '/transition' && request.method === 'POST') {
      ...
      const validNext = Order.TRANSITIONS[orderData.status] || [];
      ...
      if (body.status === 'delivered') {
        const workspaceId = this.env.WORKSPACE.idFromName(orderData.storeSlug);
        const workspaceDO = this.env.WORKSPACE.get(workspaceId);
...
/**
 * Editor — Live-preview relay for desktop editors.
 */
export class Editor extends DurableObject {
  async fetch(request: Request): Promise<Response> {
    const url = new URL(request.url);
    if (url.pathname === '/ws') {
