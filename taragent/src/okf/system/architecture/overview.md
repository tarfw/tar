...
description: tarai is a thin 3-tab Expo client; taragent is the Cloudflare Workers backend handling all business logic.
...
├── Workspace (per-workspace stock, services, config)
├── Order (per-order state machine + payment)
