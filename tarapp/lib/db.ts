...
/**
 * Seeds starting mock data if the database is newly created
 */
export async function seedMockData(db: Database) {
  try {
    ...
    if (userMatters && userMatters[0] && (userMatters[0] as any).count > 0) {
      console.log("[DB:Seed] Skipping mock seeding (already populated)");
      return;
    }
    ...
    await db.run(
      "INSERT OR REPLACE INTO matter (id, code, type, scope, title, data) VALUES (?, ?, 'note', 'p', ?, ?)",
    ...
    // Use Case 2: Product Catalogs (scope: global & personal)
    ...
    await db.run(
      "INSERT OR REPLACE INTO matter (id, code, type, scope, title, data) VALUES (?, ?, 'note', 'p', ?, ?)",
      ["technician_tools", "TOOLS", "note", "p", "Personal Tool Belt Status", JSON.stringify({ tools: ["pipe_wrench", "teflon_tape", "adjustable_spanner"] })]
    );

    console.log("[DB:Seed] Seeding completed successfully!");
  } catch (e) {
    console.error("[DB:Seed] Error seeding mock data:", e);
  }
}
