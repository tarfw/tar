...
import type { StorefrontLayout } from './storefront/schema';
...
// ============================================================
// AI PATH: /agents/:name/:id — Flue agent handles intent
// ============================================================
app.post('/agents/:name/:id', async (c) => {
  const name = c.req.param('name');
  const id = c.req.param('id');
  const body = await c.req.json();
  return c.json({ agent: name, id, message: body.message, status: 'agent endpoint ready' });
});

// ============================================================
// UI PATH: /tools/:toolName — Direct tool calls (no LLM)
// ============================================================
app.post('/tools/create_matter', async (c) => {
  const input = await c.req.json();
  const result = await createMatter(input);
  return c.json(result);
});

app.post('/tools/get_matter', async (c) => {
  const input = await c.req.json();
  const result = await getMatter(input);
  return c.json(result);
});

app.post('/tools/list_matters', async (c) => {
  const input = await c.req.json();
  const result = await listMatters(input);
  return c.json(result);
});

app.post('/tools/update_matter', async (c) => {
  const input = await c.req.json();
  const result = await updateMatter(input);
  return c.json(result);
});

app.post('/tools/append_motion', async (c) => {
  const input = await c.req.json();
  const result = await appendMotion(input);
  return c.json(result);
});

app.post('/tools/read_motions', async (c) => {
  const input = await c.req.json();
  const result = await readMotions(input);
  return c.json(result);
});

app.post('/tools/link_graph', async (c) => {
  const input = await c.req.json();
  const result = await linkGraph(input);
  return c.json(result);
});

app.post('/tools/traverse_graph', async (c) => {
  const input = await c.req.json();
  const result = await traverseGraph(input);
  return c.json(result);
});

app.post('/tools/set_attr', async (c) => {
  const input = await c.req.json();
  const result = await setAttr(input);
  return c.json(result);
});
...
app.post('/tools/store_memory', async (c) => {
  const input = await c.req.json();
  const result = await storeMemory(input);
  return c.json(result);
});

app.post('/tools/read_form', async (c) => {
  const input = await c.req.json();
  const result = await readForm(input);
  return c.json(result);
});

// ============================================================
// WORKFLOW PATH: /workflows/:name — Structured flows
// ============================================================
app.post('/workflows/checkout', async (c) => {
  const input = await c.req.json();
  const { actionCheckout } = await import('./actions/composite/checkout');
  const result = await actionCheckout.run({ input, log: console } as any);
  return c.json(result);
});

// ============================================================
// STOREFRONT: Domain routing (*.tarai.space)
// ============================================================
app.get('*', async (c) => {
  ...
  if (url.pathname === '/edit') {
    return c.html(editorShell(storeSlug));
  }
...
app.post('*', async (c) => {
  ...
  if (url.pathname === '/api/checkout') {
    ...
    return orderDO.fetch('https://do/create', {
      ...
      body: JSON.stringify({ storeSlug, items: body.items, email: body.email }),
    });
  }
