...
# Milo Design Specification
Parsed and rendered using 6 Core Layout Blocks driven by design.md.
`;

function testPipeline() {
  console.log('--- 1. Parsing design.md with designmd-parser ---');
  const startParse = performance.now();
  const plan = parseDesignMd(sampleDesignMd, 'w:milo_demo');
  const parseTime = (performance.now() - startParse).toFixed(3);
  console.log(`✅ design.md parsed in ${parseTime}ms`);
  console.log(`Routes: ${plan.routes.length}, Nodes in Home Route: ${plan.routes[0].nodes.length}`);

  console.log('--- 2. Compiling 6 Core Layout Blocks to HTML ---');
  ...
  const hasMilo = html.includes('milo');
  const hasGridCols = html.includes('--grid-cols: 3');
  const hasPromo = html.includes('100% VET EXPENSE REIMBURSEMENT');

  if (hasMilo && hasGridCols && hasPromo) {
    console.log('🎉 SUCCESS: 6 Core Layout Blocks render pixel-faithful Milo layout driven by design.md!');
  } else {
