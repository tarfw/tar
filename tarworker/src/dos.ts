 ...
 * Prevents overselling by managing stock counts and locks in-memory.
 */
export class StorefrontDO extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    // GET /stock — retrieve current stock levels
    ...
    // POST /stock/update — update stock levels
    ...
    // POST /reserve — check stock and reserve temporarily
    if (path === '/reserve' && request.method === 'POST') {
      ...
      for (const item of body.items) {
        ...
        if (currentStock < item.qty) {
          return new Response(
            JSON.stringify({ error: `Insufficient stock for ${item.name}. Available: ${currentStock}` }),
      ...
      return new Response(JSON.stringify({ ok: true, reservationId, expiresAt }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }

    // POST /commit — finalize stock deduction
    ...
    // POST /release — release reserved stock (checkout cancelled / expired)
 ...
 * Self-destructs after completion/cancel via alarms.
 */
export class OrderDO extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    // POST /create — setup order, request stock reservation
    if (path === '/create' && request.method === 'POST') {
      ...
      if (!body?.storeSlug || !body?.items) {
        return new Response('Bad request', { status: 400 });
      }

      // Check stock & reserve via StorefrontDO
      ...
      const res = await storefrontDO.fetch('https://do/reserve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Set alarm to auto-expire checkout in 10 minutes
    ...
    // POST /pay — mark as paid and finalize reservation
    if (path === '/pay' && request.method === 'POST') {
      ...
      if (orderData.status !== 'pending_payment') {
        return new Response(JSON.stringify({ error: `Cannot pay order in state ${orderData.status}` }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      }

      // Commit reservation in StorefrontDO
      const storefrontId = this.env.STOREFRONT_DO.idFromName(orderData.storeSlug);
      const storefrontDO = this.env.STOREFRONT_DO.get(storefrontId);

      const res = await storefrontDO.fetch('https://do/commit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        return new Response('Failed to commit stock reservation', { status: 500 });
      }

      orderData.status = 'paid';
      await this.ctx.storage.put('data', orderData);

      // Clear the alarm since checkout completed successfully
  ...
  // Alarm runs if payment times out
  async alarm(): Promise<void> {
    ...
    if (orderData.status === 'pending_payment') {
      ...
      await storefrontDO.fetch('https://do/release', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reservationId: orderData.reservationId }),
      });
...
/**
 * WorkspaceDO — Team sync coordinator.
...
export class WorkspaceDO extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/sync' && request.headers.get('Upgrade') === 'websocket') {
    ...
    // POST /broadcast — manual trigger to broadcast sync mutation event
  ...
  broadcast(message: string) {
    for (const ws of this.ctx.getWebSockets()) {
      try {
        ...
        // Cleaned up automatically by hibernation
      }
    }
  }
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    // Broadcast mutation to all other teammates
